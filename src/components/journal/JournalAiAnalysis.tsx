...
// ── Markdown renderer ringkas (mandiri) ──
...
function Markdown({ text }: { text: string }) {
  ...
  const flush = () => {
    ...
    els.push(<ul key={`ul${els.length}`} className="space-y-1 my-1.5">{list.map((it, i) => (
      <li key={i} className="flex gap-2 items-start text-sm leading-relaxed"><span className="text-purple-400 text-xs shrink-0 mt-1">•</span><span className="text-foreground/85">{renderInline(it)}</span></li>
    ))}</ul>)
    list = []
  }
  lines.forEach((line, i) => {
    ...
    if (t.startsWith('## ')) { flush(); els.push(<p key={i} className="text-sm font-black text-primary mt-4 mb-1 flex items-center gap-1.5">{t.slice(3)}</p>); return }
