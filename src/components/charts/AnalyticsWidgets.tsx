...
export function InsightsCard({ trades, stats, fmt }: { trades: Trade[]; stats: DashboardStats; fmt: (n: number) => string }) {
  ...
  return (
    <Card className="border-primary/20 bg-gradient-to-br from-primary/8 via-transparent to-transparent">
      <CardHeader className="pb-2">
        <CardTitle className="text-sm font-semibold flex items-center gap-2">
          <Lightbulb size={15} className="text-primary" /> Insight Otomatis
        </CardTitle>
      </CardHeader>
      <CardContent>
        <div className="space-y-2.5">
          {insights.map((ins, i) => (
            <div key={i} className="flex items-start gap-2.5">
              <ins.icon size={15} className={`${ins.color} shrink-0 mt-0.5`} />
