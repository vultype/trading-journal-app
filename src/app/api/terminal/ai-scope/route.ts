...
const FOCUS: Record<string, string> = {
  ...
  makro: `FOKUS: ANALISA MAKRO/FUNDAMENTAL. UTAMAKAN "macroCandle" di snapshot (proxy per-candle: dolar UUP & yield IEF, sudah dinyatakan sbg dampak KE EMAS −100..+100 per M5/M15/H1) untuk menilai kondisi makro intraday SAAT INI — ini sebanding horizon waktunya dgn teknikal. Field "macro" (FRED) HARIAN = konteks lambat/latar saja. Bahas dolar (DXY), yield 2Y/10Y & kurva 2s10s, real yield, inflasi (CPI, Core CPI, Core PCE, breakeven), Fed Funds & arah kebijakan Fed, pengangguran. Jelaskan implikasi masing-masing ke EMAS (dolar/yield naik = tekan emas; inflasi naik/ekspektasi pangkas bunga = dukung emas). Jika macroCandle.conflict = true (dolar & yield per-candle berlawanan arah): tegaskan kondisi TARIK-MENARIK makro → berpotensi RANGING/whipsaw, turunkan keyakinan arah. Simpulkan bias makro.`,
