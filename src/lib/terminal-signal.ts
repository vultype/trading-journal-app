...
// ~1 = tren bersih/efisien, ~0 = choppy/ranging. Self-normalizing → adaptif ke volatilitas.
export function efficiencyRatio(closes: number[], n = 14): number {
...
export function regimeOf(p: {
  bbSqueeze: boolean; adx: number; adxTrend: 'naik' | 'turun' | 'stabil'; trendUp: boolean
  er: number; diSpread: number
  m5?: { adx: number; trendUp: boolean }; h1?: { adx: number; trendUp: boolean }
  prevPhase?: RegimePhase
}): Regime {
  const { bbSqueeze, adx, adxTrend, trendUp, er, diSpread, m5, h1, prevPhase } = p
  ...
  // ── (1) Skor kekuatan tren 0..100 ──
  const adxN = clamp((adx - 12) / 18, 0, 1)     // ADX 12..30 → 0..1
  ...
  if (phase === 'trending') {
    ...
    return { label: `Trending ${Dir}`, phase, dir, c: green, strength,
      desc: koreksi
        ? `tren ${trendUp ? 'naik' : 'turun'} kuat — M5 sedang koreksi/pullback, tunggu lanjutan searah`
