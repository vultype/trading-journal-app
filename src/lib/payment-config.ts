...
export type Gateway = 'none' | 'doku' | 'ipaymu' | 'midtrans' | 'mayar' | 'manual'
...
export type PaymentConfig = {
  ...
  mayar: { apiKey: string; production: boolean }
}
...
function envFallback(): PaymentConfig {
  ...
  return {
    ...
    mayar: { apiKey: process.env.MAYAR_API_KEY || '', production: process.env.MAYAR_ENV === 'production' },
  }
}
...
type Row = {
  ...
  mayar_api_key: string | null; mayar_production: boolean
}
...
export async function getPaymentConfig(): Promise<PaymentConfig> {
  ...
  try {
    ...
    return {
      activeGateway: ['doku', 'ipaymu', 'midtrans', 'mayar', 'manual', 'none'].includes(g) ? g : 'none',
      ...
      mayar: { apiKey: s(r.mayar_api_key, env.mayar.apiKey), production: !!r.mayar_production },
    }
  } catch { return env }
}
